...
class Prune(object):
    ...
    def run(self, input_tree, taxa_to_retain_file, output_tree):
        ...
        self.logger.info('Identified %d extant taxa to retain in tree.' % len(taxa_in_tree))
        
        if taxa_to_retain:
            self.logger.warning('Failed to identify %d taxa: %s' % (len(taxa_to_retain), ','.join(taxa_to_retain)))
